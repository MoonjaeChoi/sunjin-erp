...
# streamwood 시스템 보고서 페이지 — 이식 가이드

엘리에셀 교회 관리 시스템 **streamwood** 의 기능 현황을 소개하는
...
│           └── _components/
│               ├── StreamwoodReportClient.tsx ← 메인 클라이언트 오케스트레이터
...
└── public/
    └── streamwood-screenshots/                ← 65개 PNG 스크린샷
        ├── auth/
        ├── home/
        ├── person/
        ├── attendance/
...
패키지의 `src/app/streamwood-report/` 폴더 전체를
대상 프로젝트의 `src/app/` 아래에 복사합니다.

```bash
# 패키지 디렉토리에서 실행
cp -r src/app/streamwood-report  /path/to/your-project/src/app/
```
...
패키지의 `public/streamwood-screenshots/` 폴더를
대상 프로젝트의 `public/` 아래에 복사합니다.

```bash
cp -r public/streamwood-screenshots  /path/to/your-project/public/
```

> **주의:** `public/streamwood-screenshots/` 경로가 반드시 일치해야 합니다.
> `ScreenshotViewer.tsx` 가 `/streamwood-screenshots/[경로]` 를 정적으로 참조합니다.
...
브라우저에서 `http://localhost:3000/streamwood-report` 접속.
...
const navItems = [
  // ...
  { label: 'streamwood 보고서', href: '/streamwood-report', icon: BookOpen },
];
```
...
```typescript
// src/app/streamwood-report/layout.tsx (인증 추가 예시 — NextAuth v5)
...
|------|------------|
| `layout.tsx` | 없음 |
| `page.tsx` | `StreamwoodReportClient` |
| `StreamwoodReportClient.tsx` | `modules.ts`, 4개 컴포넌트 |
...
```typescript
// modules.ts 구조
export interface StreamwoodSection {
  id: string;           // "1-1", "2-3" 형식
  title: string;
  description: string;  // Markdown 사용 가능
  screenshotPath?: string; // "home/01_home.png" 형식 (public/streamwood-screenshots/ 기준)
  isBeta?: boolean;
}

export interface StreamwoodModule {
...
□ src/app/streamwood-report/ 폴더 복사
□ public/streamwood-screenshots/ 폴더 복사
□ http://localhost:3000/streamwood-report 접속 확인
□ 스크린샷 이미지 정상 표시 확인
```
