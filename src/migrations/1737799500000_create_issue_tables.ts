...
export class CreateIssueTables1737799500000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE ISSUE_ATTACHMENT ADD CONSTRAINT FK_ISSUE_ATTACHMENT_ISSUE FOREIGN KEY (issue_id) REFERENCES ISSUE(id) ON DELETE RESTRICT`
    );
    await queryRunner.query(
      `ALTER TABLE ISSUE_ATTACHMENT ADD CONSTRAINT FK_ISSUE_ATTACHMENT_UPLOADED_BY FOREIGN KEY (uploaded_by_id) REFERENCES EMPLOYEE(id) ON DELETE RESTRICT`
    );
    ...
    await queryRunner.query(`CREATE INDEX IDX_ISSUE_HISTORY_CHANGED_BY ON ISSUE_HISTORY(changed_by_id)`);
    await queryRunner.query(`CREATE INDEX IDX_ISSUE_HISTORY_CHANGED_AT ON ISSUE_HISTORY(changed_at)`);

    // 10. ISSUE_HISTORY 외래키 추가
    await queryRunner.query(
      `ALTER TABLE ISSUE_HISTORY ADD CONSTRAINT FK_ISSUE_HISTORY_ISSUE FOREIGN KEY (issue_id) REFERENCES ISSUE(id) ON DELETE RESTRICT`
    );
    await queryRunner.query(
      `ALTER TABLE ISSUE_HISTORY ADD CONSTRAINT FK_ISSUE_HISTORY_CHANGED_BY FOREIGN KEY (changed_by_id) REFERENCES EMPLOYEE(id) ON DELETE RESTRICT`
    );
  }
