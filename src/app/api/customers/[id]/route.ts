...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const customers = await queryRunner.query(sql, { id });
...
export async function PUT(
  ...
  try {
    ...
    try {
      ...
      const customers = await queryRunner.query(existingSql, { id });
      ...
      const updatedCustomers = await queryRunner.query(existingSql, { id });
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      `;
      const projectCount = await queryRunner.query(projectCountSql, {
        customerId: id,
      });
      const projects = parseInt(projectCount[0]?.count || '0', 10);
      ...
      const techSupportCountSql = `
        ...
        WHERE customer_id = :customerId AND deleted_at IS NULL
      `;
      const techSupportCount = await queryRunner.query(techSupportCountSql, {
        customerId: id,
      });
      const techSupports = parseInt(techSupportCount[0]?.count || '0', 10);
      ...
      const maintenanceCountSql = `
        ...
        WHERE customer_id = :customerId AND deleted_at IS NULL
      `;
      const maintenanceCount = await queryRunner.query(maintenanceCountSql, {
        customerId: id,
      });
      const maintenanceContracts = parseInt(maintenanceCount[0]?.count || '0', 10);
