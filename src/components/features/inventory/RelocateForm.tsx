...
import type { RelocateInventoryRequest } from '@/types/inventory';
...
export default function RelocateForm({ open, onClose }: RelocateFormProps) {
  const [form, setForm] = useState<FormState>(getInitialForm());
  const [errors, setErrors] = useState<Record<string, string>>({});
  const { data: listData } = useInventoryListQuery({ statuses: ['재고', '출고', '고장'] });
  const { mutateAsync: relocate, isPending } = useRelocateInventoryMutation(form.inventory_id || 0);
  ...
  const validate = (): boolean => {
    ...
    if (!form.current_location.trim()) {
      newErrors.current_location = '새 위치를 입력하세요.';
    } else if (form.current_location.length > 200) {
      newErrors.current_location = '새 위치는 200자 이하로 입력하세요.';
    }
  ...
  const handleSubmit = async () => {
    ...
    const data: RelocateInventoryRequest = {
    ...
    try {
      await relocate(data);
      toast.success('위치 변경이 완료되었습니다.');
      onClose();
    } catch (error: any) {
      toast.error(error.message || '위치 변경에 실패했습니다.');
    }
  };
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium">
              새 위치 <span className="text-destructive">*</span>
            </label>
            <Input
              value={form.current_location}
              onChange={(e) => setForm({ ...form, current_location: e.target.value })}
              placeholder="새로운 위치를 입력하세요"
              maxLength={200}
            />
